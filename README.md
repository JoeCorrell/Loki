...
Three sources are merged field by field, and anything you edit by hand is never overwritten. ScreenScraper leads: it identifies a ROM by its CRC, MD5 and SHA1 rather than by its name, and supplies titles, credits, genres, dates, synopses and artwork for exactly the systems this launcher is for. When a file's hashes and name are both unknown to it, the title is searched for instead and the results are scored rather than trusted. SteamGridDB supplies the square grid image a cell wants — the one thing ScreenScraper has no asset for — with your API key. Wikidata supplies developer, publisher, dates and series and needs no account at all, so it is what answers with nothing signed in to. A source without a credential is skipped rather than being fatal, and each artwork slot asks whichever provider is good at it.
